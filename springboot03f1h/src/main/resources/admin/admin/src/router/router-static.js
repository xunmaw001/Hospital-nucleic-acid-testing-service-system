import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import yimiaojiezhong from '@/views/modules/yimiaojiezhong/list'
    import xitonggonggao from '@/views/modules/xitonggonggao/list'
    import putongguanliyuan from '@/views/modules/putongguanliyuan/list'
    import yihurenyuan from '@/views/modules/yihurenyuan/list'
    import jiezhongjindu from '@/views/modules/jiezhongjindu/list'
    import yonghu from '@/views/modules/yonghu/list'
    import tongzhigonggao from '@/views/modules/tongzhigonggao/list'
    import jiezhongdingdan from '@/views/modules/jiezhongdingdan/list'
    import jiancejindu from '@/views/modules/jiancejindu/list'
    import hesuanjiance from '@/views/modules/hesuanjiance/list'
    import config from '@/views/modules/config/list'
    import jiancedingdan from '@/views/modules/jiancedingdan/list'
    import hesuanjiancezhengming from '@/views/modules/hesuanjiancezhengming/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
          ,{
	path: '/yimiaojiezhong',
        name: '疫苗接种',
        component: yimiaojiezhong
      }
          ,{
	path: '/xitonggonggao',
        name: '系统公告',
        component: xitonggonggao
      }
          ,{
	path: '/putongguanliyuan',
        name: '普通管理员',
        component: putongguanliyuan
      }
          ,{
	path: '/yihurenyuan',
        name: '医护人员',
        component: yihurenyuan
      }
          ,{
	path: '/jiezhongjindu',
        name: '接种进度',
        component: jiezhongjindu
      }
          ,{
	path: '/yonghu',
        name: '用户',
        component: yonghu
      }
          ,{
	path: '/tongzhigonggao',
        name: '通知公告',
        component: tongzhigonggao
      }
          ,{
	path: '/jiezhongdingdan',
        name: '接种订单',
        component: jiezhongdingdan
      }
          ,{
	path: '/jiancejindu',
        name: '检测进度',
        component: jiancejindu
      }
          ,{
	path: '/hesuanjiance',
        name: '核酸检测',
        component: hesuanjiance
      }
          ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
          ,{
	path: '/jiancedingdan',
        name: '检测订单',
        component: jiancedingdan
      }
          ,{
	path: '/hesuanjiancezhengming',
        name: '核酸检测证明',
        component: hesuanjiancezhengming
      }
        ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
